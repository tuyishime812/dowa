...
function navigateToSection(section) {
    ...
    // Show appropriate sections with parallel loading
    if (section === 'home') {
        document.querySelector('.hero').style.display = 'flex';
        sections.home.forEach(id => {
            document.getElementById(id).style.display = 'block';
        });
        // Load data in parallel for faster homepage load
        Promise.all([
            loadFeaturedTracks(),
            loadLatestTracks()
        ]);
...
// Clear cache when needed (e.g., after upload)
...
async function loadFeaturedTracks() {
    ...
    container.innerHTML = createTrackSkeletons(8);
...
async function loadAllTracks() {
    ...
    // Show skeleton loaders immediately
    container.innerHTML = createTrackListSkeletons(20);
...
async function loadArtists() {
    ...
    // Show skeleton loaders immediately
    container.innerHTML = createArtistSkeletons(6);

    const artists = await fetchAPI('/artists');
    if (artists) {
        container.innerHTML = artists.map(artist => `
            <div class="artist-card">
                <img src="${artist.image_url || 'assets/images/logo.png'}" alt="${artist.name}" class="artist-image" loading="lazy" onerror="this.src='assets/images/logo.png'">
...
function createTrackCard(track, index) {
    ...
    return `
        <div class="track-card" onclick="playTrackFromQueue(${index}, 'featured')">
            ...
            <img src="${track.cover_url || 'assets/images/logo.png'}" alt="${track.title}" class="track-cover" loading="lazy" onerror="this.src='assets/images/logo.png'">
...
function createTrackListItem(track, index) {
    ...
    return `
        <div class="track-item" onclick="playTrackFromQueue(${index}, 'all')">
            <img src="${track.cover_url || 'assets/images/logo.png'}" alt="${track.title}" class="track-item-image" loading="lazy" onerror="this.src='assets/images/logo.png'">
...
function createTrackSkeletons(count) {
    return Array(count).fill(0).map(() => `
        <div class="track-card skeleton">
            <div class="track-cover skeleton-bg"></div>
            <div class="track-info">
...
function createTrackListSkeletons(count) {
    return Array(count).fill(0).map(() => `
        <div class="track-item skeleton">
            <div class="track-item-image skeleton-bg"></div>
            <div class="track-item-info">
                <div class="skeleton-text skeleton-title"></div>
                <div class="skeleton-text skeleton-artist"></div>
            </div>
...
function createArtistSkeletons(count) {
    return Array(count).fill(0).map(() => `
        <div class="artist-card skeleton">
            <div class="artist-image skeleton-bg"></div>
            <div class="skeleton-text skeleton-title"></div>
