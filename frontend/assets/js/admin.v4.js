...
function initializeEventListeners() {
    ...
    document.getElementById('settingsForm')?.addEventListener('submit', handleChangePassword);
...
async function handleUpload(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            e.target.reset();
            // Clear cache to show new track immediately
...
async function handleChangePassword(e) {
    ...
    try {
        ...
        if (response.ok) {
            showToast('Password changed! Update Render environment variable for permanent change.', 'success');
            // Clear form
            document.getElementById('currentPassword').value = '';
            document.getElementById('newPassword').value = '';
            document.getElementById('confirmPassword').value = '';
        } else {
            showToast(result.detail || 'Password change failed', 'error');
        }
    } catch (error) {
        showToast('Failed to change password. Make sure backend is running.', 'error');
    }
}
    document.getElementById('trackSearch').value = query;
    filterTracks(query);
